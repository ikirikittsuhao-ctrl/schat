...
function signToken(user) {
  return jwt.sign(
    ...
    { expiresIn: process.env.JWT_EXPIRES_IN || "7d", issuer: "schat" }
  );
}
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (!validUsername(username)) {
      return res.status(400).json({ error: "ユーザー名は英数字と _ の3〜24文字で入力してください。" });
    }
    if (password.length < 8 || password.length > 128) {
      return res.status(400).json({ error: "パスワードは8〜128文字にしてください。" });
    }
    if (!validDisplayName(displayName)) {
      return res.status(400).json({ error: "表示名は1〜40文字です。" });
    }
...
app.post("/api/rooms", requireAuth, async (req, res) => {
  try {
    ...
    const { error: memberError } = await supabase
      .from("room_members")
      .insert({ room_id: room.id, user_id: req.user.id, role: "owner" });
...
app.post("/api/friends/add", requireAuth, async (req, res) => {
  try {
    ...
    const { data: existingRooms, error: roomError } = await supabase
      .from("room_members")
      .select("room_id, rooms:room_id(id,name,type,owner_id,created_at,updated_at)")
    ...
    for (const item of existingRooms || []) {
      ...
      if (ids.length === 2 && ids.includes(req.user.id) && ids.includes(friend.id)) {
        room = candidate;
        break;
      }
    }
    ...
    io.emit("rooms:changed");
    res.status(201).json({ friend, room });
  } catch (error) {
...
app.post("/api/friends/qr", requireAuth, async (req, res) => {
  try {
    const token = safeText(req.body.token, 200);
    ...
    if (!room) {
      const { data: newRoom, error } = await supabase
        .from("rooms")
        .insert({ name: friend.display_name, type: "friend", owner_id: req.user.id })
        .select("id, name, type, owner_id, created_at, updated_at")
        .single();
      if (error) throw error;
...
app.get("/api/friends/qr", requireAuth, (req, res) => {
  const token = jwt.sign(
    { sub: req.user.id, type: "friend_qr", nonce: crypto.randomUUID() },
    process.env.JWT_SECRET,
    { issuer: "schat-qr", expiresIn: "10m" }
  );
  res.json({ token });
});
